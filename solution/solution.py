...
class RAGASEvaluator:
    ...
    def evaluate_context_recall(self, contexts: list[str], expected: str) -> float:
        ...
        for context in contexts:
            union_tokens |= _tokenize(context)
    ...
    def evaluate_context_precision(
        ...
        # Calculate rank-aware Average Precision
        relevant_count = 0
        precision_sum = 0.0
        ...
        for k, context in enumerate(contexts, start=1):
            ...
            overlap = context_tokens & expected_tokens
            relevance_ratio = len(overlap) / len(expected_tokens)
        ...
        # If no relevant chunks found, return 0.0
        if relevant_count == 0:
            return 0.0

        # Calculate Average Precision
        ap_score = precision_sum / relevant_count
    ...
    def run_full_eval(
        ...
        """
        # Calculate three answer-side metrics
        ...
        # Initialize retrieval metrics as None
        context_recall = None
        context_precision = None

        # Calculate retrieval metrics if contexts provided
...
def rerank_by_overlap(contexts: list[str], query: str) -> list[str]:
    ...
    """
    def overlap_score(chunk: str) -> int:
...
Evaluation Rubric (score each criterion from 0 to 1):
{rubric_str}
...
Score scale: 0 = very poor, 0.5 = acceptable, 1 = excellent.
"""
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for i, result in enumerate(scores_batch):
            ...
            for score in scores.values():
                ...
                if i == 0:
                    first_position_scores.append(score)
                else:
                    other_position_scores.append(score)

        # Check leniency and severity bias
        ...
        first_avg = sum(first_position_scores) / len(first_position_scores) if first_position_scores else 0
        other_avg = sum(other_position_scores) / len(other_position_scores) if other_position_scores else 0
        positional_bias = (first_avg - other_avg) > 0.1 if (first_position_scores and other_position_scores) else False
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        # Calculate averages for new results
        def calc_avg(results: list[EvalResult], field: str) -> float:
            if not results:
                return 0.0
            return sum(getattr(r, field) for r in results) / len(results)

        new_avg_f = calc_avg(new_results, "faithfulness")
        new_avg_r = calc_avg(new_results, "relevance")
        new_avg_c = calc_avg(new_results, "completeness")

        baseline_avg_f = calc_avg(baseline_results, "faithfulness")
        baseline_avg_r = calc_avg(baseline_results, "relevance")
        baseline_avg_c = calc_avg(baseline_results, "completeness")

        # Check for regressions (drop > 0.05)
        regressions = []
        if new_avg_f < baseline_avg_f - 0.05:
            regressions.append("faithfulness")
        if new_avg_r < baseline_avg_r - 0.05:
            regressions.append("relevance")
        if new_avg_c < baseline_avg_c - 0.05:
        ...
        return {
            ...
            "baseline_avg_faithfulness": baseline_avg_f,
            "baseline_avg_relevance": baseline_avg_r,
            "baseline_avg_completeness": baseline_avg_c,
    ...
    def identify_failures(
        ...
        for result in results:
            # Check if any of the three answer-side scores is below threshold
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        # Count how many scores are notably low (below 0.5)
        low_scores_count = sum(1 for s in scores.values() if s < 0.5)

        # If multiple issues, return that
        if low_scores_count >= 2:
            return "Multiple issues detected — review full pipeline"

        # Map lowest score to root cause
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, failure in enumerate(failures):
            ...
            suggestion = suggestions[i] if i < len(suggestions) else "Review and fix manually"
    ...
    def generate_improvement_suggestions(
        ...
        # Hallucination suggestions (faithfulness issues)
        if hallucination_count > 0:
            suggestions.append(
                "Implement hallucination checker to filter unsupported claims"
            )
            suggestions.append(
                "Add grounding guardrails to ensure answers are sourced from retrieved context"
            )

        # Relevance suggestions
        if irrelevant_count > 0:
            suggestions.append(
                "Improve prompt clarity and intent classification"
            )
            suggestions.append(
                "Add clarifying questions when user intent is ambiguous"
            )
        ...
        if incomplete_count > 0:
            ...
            suggestions.append(
                "Add few-shot examples showing complete answers"
            )

        # Off-topic suggestions
        if off_topic_count > 0:
            suggestions.append(
                "Improve scope detection and intent routing"
            )
            suggestions.append(
                "Add explicit out-of-scope handling with helpful redirects"
            )

        # If we have failures but no specific patterns, add generic suggestions
        if not suggestions:
            suggestions.append("Review full pipeline for general quality issues")
            suggestions.append("Consider retraining or fine-tuning the model")
            suggestions.append("Audit retrieval quality and relevance scoring")

        # Return at least 3 suggestions
        if len(suggestions) < 3:
            suggestions.extend([
                "Monitor metrics continuously in production",
                "Set up regression tests for each deployment",
                "Implement A/B testing for prompt changes",
            ])

        return suggestions[:10]  # Limit to reasonable number
